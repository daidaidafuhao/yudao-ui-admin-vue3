      ...
      >
        ...
        <el-table-column align="center" label="配送方式" width="120">
          <template #default>
            ...
            <!-- 显示无人机配送信息 -->
            <div v-if="scope.row.logisticsId === -1 && scope.row.logisticsNo?.startsWith('DRONE-')" class="mt-1">
              <el-tag type="info" size="small">
                <Icon icon="ep:guide" class="mr-1" />
                {{ scope.row.logisticsNo.replace('DRONE-', '') }}
              </el-tag>
            </div>
          </template>
        </el-table-column>
...
const headerStyle = ({ row, columnIndex }: any): any => {
  // 表头第一行第一列占 8
  if (columnIndex === 0) {
